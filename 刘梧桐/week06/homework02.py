...
print("\n--- 插入数据 ---")
# 实例化模型对象
mark = Author(name='马克·吐温', nationality='351 Farmington Avenue, Hartford, Connecticut, 06105, USA')
jan = Author(name='简·奥斯汀', nationality='Chawton, Alton, Hampshire, GU34 1SD, UK')
ost = Author(name='欧内斯特·海明威', nationality='907 Whitehead Street, Key West, Florida, 33040, USA')


# 将对象添加到会话中
session.add_all([mark, jan, ost])

# 插入书籍数据，通过对象关系来设置作者
book_ost = Book(title='老人与海', published_year=1952, author=ost)
book_jan = Book(title='傲慢与偏见', published_year=1813, author=jan)
book_mark = Book(title='哈克贝利·费恩历险记', published_year=1884, author=mark)

session.add_all([book_ost, book_jan, book_mark])

# 插入借阅人数据
# borrower_wangwu = Borrower(name='wangwu', email='wangwu@example.com')
# borrower_laoliu = Borrower(name='laoliu', email='laoliu@example.com')
# session.add_all([borrower_wangwu, borrower_laoliu])
...
print("\n--- 更新书籍信息 ---")
# 查询要更新的对象
book_to_update = session.query(Book).filter_by(title='呐喊').first()
if book_to_update:
    book_to_update.published_year = 1998
    session.commit()
    print("书籍 '呐喊' 的出版年份已更新。")

# 再次查询，验证更新
updated_book = session.query(Book).filter_by(title='呐喊').first()
if updated_book:
    print(f"更新后的信息: 书籍: {updated_book.title}, 出版年份: {updated_book.published_year}")

print("\n--- 删除借阅人 ---")
# 查询要删除的对象
borrower_to_delete = session.query(Borrower).filter_by(name='lisi').first()
