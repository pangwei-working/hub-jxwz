...
dataset_df = pd.read_csv('./作业数据-waimai_10k.csv', sep=',')
# print(dataset_df.head(5))

# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
# labels = lbl.fit_transform(dataset_df[0].values[:10000])
labels = lbl.fit_transform(dataset_df['label'].values[3800:4200])
# 提取前500个文本内容
# texts = list(dataset_df[1].values[:10000])
texts = list(dataset_df['review'].values[3800:4200])
...
# 下载命令 modelscope download --model google-bert/bert-base-chinese --local_dir ./assets/models/bert-base-chinese
...
model = BertForSequenceClassification.from_pretrained(
    './assets/models/bert-base-chinese', num_labels=2 # 判别式模型，分多少类
)
