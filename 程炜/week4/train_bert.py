...
# 加载和预处理数据
dataset_df = pd.read_csv('../assets/dataset/作业数据-waimai_10k.csv', sep=',', header=None)

# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
labels = lbl.fit_transform(dataset_df[0].values[1:1000])
# 提取前500个文本内容
texts = list(dataset_df[1].values[1:1000])
