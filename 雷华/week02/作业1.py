...
dataset = pd.read_csv("D:\\nlp20\\week1\\dataset.csv", sep="\t", header=None)
...
#        self.fc2 = nn.Linear(hidden_dim, output_dim)
        super(SimpleClassifier, self).__init__()
        self.fc1 = nn.Linear(input_dim, 512)  # 第1层：512节点
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Linear(512, 256)  # 第2层：256节点
        self.relu2 = nn.ReLU()
        self.fc3 = nn.Linear(256, 128)   # 第3层：128节点
        self.relu3 = nn.ReLU()
        self.fc4 = nn.Linear(128, output_dim)# 第4层：输出层
    def forward(self, x):
        # 手动实现每层的计算
#        out = self.fc1(x)
#        out = self.relu(out)
...
model = SimpleClassifier(vocab_size, output_dim) # 维度和精度有什么关系？
