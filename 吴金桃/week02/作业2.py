...
dataset = pd.read_csv("D:\Code\Study_AI\dataset.csv", sep="\t", header=None)
...
# 6. 绘制结果
with torch.no_grad():
    y_predicted = torch.zeros(vocab_size) * vocab_size + max_len

plt.figure(figsize=(10, 6))
plt.scatter(model, char_to_index, label='Raw data', color='blue', alpha=0.6)
plt.plot(model, y_predicted, label=f'Model: y = {index_to_label:.2f}x + {vocab_size:.2f}', color='red', linewidth=2)
plt.xlabel('X')
plt.ylabel('y')
plt.legend()
plt.grid(True)
