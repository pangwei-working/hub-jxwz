...
texts = dataset_df["review"].to_list()
...
def compute_metrics(eval_pred):
    ...
    return {
        ...
        "f1": f1_score(labels, predictions, average="weighted")  # 或 "macro"
        }
...
training_args = TrainingArguments(
    ...
    metric_for_best_model='f1',  # 根据哪个指标选择最佳模型（如 accuracy）
    greater_is_better=True,  # 指标越大越好（如 accuracy）
    save_total_limit=3,  # 保留最多 3 个 checkpoint
)
...
tokenizer.save_pretrained("./results/best_model")   # ✅ 额外保存 tokenizer 文件