...
dataset = pd.read_csv("作业数据-waimai_10k.csv", sep=",", header=0)
print(dataset)  # 数据集维度（n,2）n行2列

# 初始化并拟合标签编码器，将文本标签（如“体育”）转换为数字标签（如0, 1, 2...）
lbl = LabelEncoder()
# 根据数据集的标签列生成标签,当有表头时，需配合表头名称获取数据列
...
x_train, x_test, train_label, test_label = train_test_split(
    list(dataset["review"].values[:500]),
    lbl.transform(dataset["label"].values[:500]),
    test_size=0.2,
    stratify=dataset["label"][:500].values
)
labels = lbl.fit_transform(dataset["label"].values[:500])

BERT_MODEL_PERTRAINED_PATH = r"D:\workstation\project_practice\01-intent-classify\assets\models\bert-base-chinese"
...
# Bert最长输入token是512，减去特殊字符，则大致510字符。
...
model = BertForSequenceClassification.from_pretrained(BERT_MODEL_PERTRAINED_PATH, num_labels=len(labels))
