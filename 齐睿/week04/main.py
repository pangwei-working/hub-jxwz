...
from model.bert import model_for_bert, model_for_takeout_bert
...
@app.post("/v1/text-cls-takeout/bert")
def bert_classify(req: TextClassifyRequest) -> TextClassifyResponse:
    """
    利用TFIDF进行文本分类
    ...
    # info 日志
    try:
        response.classify_result = model_for_takeout_bert(req.request_text)
        response.error_msg = "ok"
    except Exception as err:
        # error 日志
        response.classify_result = ""
