...
# 定义不同层数和节点个数的模型
class SimpleClassifier1(nn.Module):  # 原始模型：1层128节点
...
class SimpleClassifier2(nn.Module):  # 更宽模型：1层256节点
    def __init__(self, input_dim, hidden_dim, output_dim):
...
class DeepClassifier(nn.Module):  # 更深模型：2层128-64节点
...
# 训练不同模型并对比loss
models = [
    ("1层128节点", SimpleClassifier1(vocab_size, 128, output_dim)),
    ("1层256节点", SimpleClassifier2(vocab_size, 256, output_dim)),
    ("2层128-64节点", DeepClassifier(vocab_size, 128, 64, output_dim))
]
...
for name, losses in results.items():
    final_loss = losses[-1]
    print(f"{name}: 最终Loss = {final_loss:.4f}")
