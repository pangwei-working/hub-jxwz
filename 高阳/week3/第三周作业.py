...
class GRUClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
        ...
        self.gru = nn.GRU(embedding_dim, hidden_dim, batch_first=True)  # 替换LSTM为GRU
        self.fc = nn.Linear(hidden_dim, output_dim)

    def forward(self, x):
        embedded = self.embedding(x)
        gru_out, hidden_state = self.gru(embedded)  # GRU输出不包含细胞状态
        out = self.fc(hidden_state.squeeze(0))  # 取最后一个时间步的隐藏状态
        return out
