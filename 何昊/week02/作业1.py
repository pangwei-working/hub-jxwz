...
class ComplexClassifier(nn.Module):
    def __init__(self, input_dim, hidden_1_dim, hidden_2_dim, hidden_3_dim, output_dim): # 层的个数 和 验证集精度
        # 层初始化
...
complex_model = ComplexClassifier(vocab_size, 32, 64, 8, output_dim)

criterion = nn.CrossEntropyLoss() # 损失函数 内部自带激活函数，softmax
optimizer = optim.SGD(model.parameters(), lr=0.05)
optimizer_o = optim.SGD(complex_model.parameters(), lr=0.05)
...
for epoch in range(num_epochs): # 12000， batch size 100 -》 batch 个数： 12000 / 100
    ...
    complex_model.train()
    running_loss = 0.0
    complex_running_loss = 0.0
    for idx, (inputs, labels) in enumerate(dataloader):
        optimizer.zero_grad()
        optimizer_o.zero_grad()
        outputs = model(inputs)
        outputs_o = complex_model(inputs)
        loss = criterion(outputs, labels)
        loss_o = criterion(outputs_o, labels)
        loss.backward()
        loss_o.backward()
        optimizer.step()
        optimizer_o.step()
        running_loss += loss.item()
        complex_running_loss += loss_o.item()
        if idx % 50 == 0:
            print(f"Batch 个数 {idx}, simple Loss: {loss.item()}, complex Loss: {loss_o.item()}")

    print(f"Epoch [{epoch + 1}/{num_epochs}],"
          f" Loss: {running_loss / len(dataloader):.4f}, "
          f"complex Loss:{complex_running_loss / len(dataloader):.4f}")
