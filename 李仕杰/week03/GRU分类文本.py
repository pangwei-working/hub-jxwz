...
class GRUClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
        ...
        self.embedding = nn.Embedding(vocab_size, embedding_dim)  # 内部的“查找表”，它是一个可以访问和操作的权重矩阵，随机初始化且可训练
    ...
    def forward(self, x):
        ...
        # print(embedded.shape)  # torch.Size([32, 40, 64])
        # print(gru_out.shape)  # torch.Size([32, 40, 128])
        # print(hidden_state.shape)  # torch.Size([1, 32, 128])
        # 因为是输入一句话（40），输出一个类别，因此用hidden而不用每字输入都有输出的gru_out
        return out
...
# print(embedding_dim, hidden_dim, output_dim)  # 64 128 12类
