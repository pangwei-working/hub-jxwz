...
# 加载和预处理数据
dataset_df = pd.read_csv("../data/作业数据-waimai_10k.csv", sep=",", header=0)
dataset_df_shuffled = dataset_df.sample(frac=1, random_state=55).reset_index(drop=True)
print(dataset_df_shuffled)
# 提取前数字标签
labels = dataset_df_shuffled['label'].values[:500]
# 提取前文本内容
texts = list(dataset_df_shuffled['review'].values[:500])
...
# 训练完成后，创建保存模型的目录
output_dir = "../saved_models/fine-tuned-bert"
import os
